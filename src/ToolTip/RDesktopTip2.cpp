...
RDesktopTip2 *RDesktopTip2::getInstance()
{
    if (!instance) {
...
int RDesktopTip2::getSystemTaskBarHeight()
{
    return std::abs(this->screen()->geometry().height()
                    - this->screen()->availableGeometry().height());
}

void RDesktopTip2::showAtSpecifiedLocation(const QPoint &position)
{
    this->move(position);
}

void RDesktopTip2::showAtSpecifiedLocation(int posx, int posy)
{
    this->move(posx, posy);
}
...
void RDesktopTip2::initAnimation()
{
    ...
    const int posy = rect.height() - this->height() - getSystemTaskBarHeight();
